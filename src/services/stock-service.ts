import type { Stock, StockQuote, StockChart, StockSearch, StockOverview, ApiResponse } from '../types';
...
class StockService {
  ...
  async searchStocks(keywords: string): Promise<ApiResponse<StockSearch[]>> {
    ...
    const searchResults: StockSearch[] = matches.map((match: any) => ({
  ...
  async getStockChart(symbol: string, interval: 'daily' | 'weekly' | 'monthly' = 'daily'): Promise<ApiResponse<StockChart[]>> {
    const functionMap = {
  ...
  async getStockOverview(symbol: string): Promise<ApiResponse<StockOverview>> {
    ...
    if (response.error) {
      return response as ApiResponse<StockOverview>;
    }
    ...
    const stockOverview: StockOverview = {
      ...
      quarterlyEarningsGrowthYOY: parseFloat(overview.QuarterlyEarningsGrowthYOY) || 0,
      quarterlyRevenueGrowthYOY: parseFloat(overview.QuarterlyRevenueGrowthYOY) || 0,
      ...
      shortPercentOutstanding: parseFloat(overview.ShortPercentOutstanding) || 0,
      ...
      forwardAnnualDividendRate: parseFloat(overview.ForwardAnnualDividendRate) || 0,
      forwardAnnualDividendYield: parseFloat(overview.ForwardAnnualDividendYield) || 0,
  ...
  async getPopularStocks(): Promise<ApiResponse<Stock[]>> {
    ...
    try {
      ...
      return {
        data: [],
        error: error instanceof Error ? error.message : 'Failed to fetch popular stocks',
        timestamp: new Date().toISOString()
      };
    }
  }
